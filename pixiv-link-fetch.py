...
parser = argparse.ArgumentParser(
    prog="pixiv-link-fetch",
    description="Returns original pixiv links given files with a pixiv filename inside a directory",
)
parser.add_argument(
    "-O",
    "--output",
        ...
        if auto:
            ...
            if cnt % limit == 0 and cnt < link_quantity:
                ...
                if pause and pause[0] == "N":
                    print(
                        f"\033[F\r\033[K^ Last accessed - visited {cnt} links ^",
                        end="\n\n",
                    )
                    auto = False
